...
export const createZeroApi = <T>(
  ...
  let obj = new Proxy({} as any, {
    get(target, name: string) {
      if (!target[name]) {
        if (name == "get" || /post/.test(name.toLocaleLowerCase())) {
          target[name] = (body: any) => {
            return req.get(preUrl + "/" + (name as string), body);
          };
        } else if (name == "put" || /put/.test(name.toLocaleLowerCase())) {
          target[name] = (body: any) => {
            return req.put(preUrl + "/" + (name as string), body);
          };
        } else if (name == "del" || /delete/.test(name.toLocaleLowerCase())) {
          target[name] = (body: any) => {
            return req.del(preUrl + "/" + (name as string), body);
          };
        } else {
          target[name] = (body: any) => {
